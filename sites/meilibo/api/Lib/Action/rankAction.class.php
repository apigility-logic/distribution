...
{
    public function presideDay()
    {
        $emceeRank_day = D('Beandetail')->query('SELECT uid,sum(bean) as total FROM `ss_beandetail` where type="income" and action="getgift" and date_format(FROM_UNIXTIME(addtime),"%m-%d-%Y")=date_format(now(),"%m-%d-%Y") group by uid order by total desc LIMIT 10');
        $a = 0;
        $ret = array();
        foreach ($emceeRank_day as $k => $vo) {
            $userinfo = D("Member")->find($vo['uid']);
            $ret[$a]['userinfo'] = array('nickname'=>$userinfo['nickname'],'curroomnum'=>$userinfo['curroomnum'],'ucuid'=>$userinfo['ucuid']);
            $emceelevel = getEmceelevel($userinfo['earnbean']);
            $ret[$a]['emceelevel'] = $emceelevel;
            $a++;
        }
        echo json_encode($ret);
    }
    public function presideWeek()
    {
        $emceeRank_week = D('Beandetail')->query('SELECT uid,sum(bean) as total FROM `ss_beandetail` where type="income" and action="getgift" and date_format(FROM_UNIXTIME(addtime),"%Y")=date_format(now(),"%Y") and date_format(FROM_UNIXTIME(addtime),"%u")=date_format(now(),"%u") group by uid order by total desc LIMIT 10');
        $a = 0;
        $ret = array();
        foreach ($emceeRank_week as $k => $vo) {
            $userinfo = D("Member")->find($vo['uid']);
            //$ret[$a]['userinfo'] = array('nickname'=>$userinfo['nickname'],'curroomnum'=>$userinfo['curroomnum'],'ucuid'=>$userinfo['ucuid']);
            $ret[$a]['userinfo'] = array('nickname'=>$userinfo['nickname'],'curroomnum'=>$userinfo['curroomnum'],'id'=>$userinfo['id']);
            $emceelevel = getEmceelevel($userinfo['earnbean']);
            $ret[$a]['emceelevel'] = $emceelevel;
            $a++;
        }
    ...
    public function presideMonth()
    {
        $emceeRank_month = D('Beandetail')->query('SELECT uid,sum(bean) as total FROM `ss_beandetail` where type="income" and action="getgift" and date_format(FROM_UNIXTIME(addtime),"%m-%Y")=date_format(now(),"%m-%Y") group by uid order by total desc LIMIT 10');
        $a = 0;
        $ret = array();
    ...
    public function presideTotal()
    {
        $emceeRank_all = D('Beandetail')->query('SELECT uid,sum(bean) as total FROM `ss_beandetail` where type="income" and action="getgift" group by uid order by total desc LIMIT 10');
        $a = 0;
        $ret = array();
    ...
    public function popularityDay()
    {
        $rqRank_day = D('Liverecord')->query('SELECT uid,sum(entercount) as total FROM `ss_liverecord` where date_format(FROM_UNIXTIME(starttime),"%m-%d-%Y")=date_format(now(),"%m-%d-%Y") group by uid order by total desc LIMIT 10');
        $a = 0;
        $ret = array();
    ...
    public function popularityWeek()
    {
        $rqRank_week = D('Liverecord')->query('SELECT uid,sum(entercount) as total FROM `ss_liverecord` where date_format(FROM_UNIXTIME(starttime),"%Y")=date_format(now(),"%Y") and date_format(FROM_UNIXTIME(starttime),"%u")=date_format(now(),"%u") group by uid order by total desc LIMIT 10');
        $a = 0;
        $ret = array();
        foreach ($rqRank_week as $k => $vo) {
            $userinfo = D("Member")->find($vo['uid']);
            //$ret[$a]['userinfo'] = array('nickname'=>$userinfo['nickname'],'curroomnum'=>$userinfo['curroomnum'],'ucuid'=>$userinfo['ucuid']);
            $ret[$a]['userinfo'] = array('nickname'=>$userinfo['nickname'],'curroomnum'=>$userinfo['curroomnum'],'id'=>$userinfo['id']);
            $emceelevel = getEmceelevel($userinfo['earnbean']);
    ...
    public function popularityMonth()
    {
        $rqRank_month = D('Liverecord')->query('SELECT uid,sum(entercount) as total FROM `ss_liverecord` where date_format(FROM_UNIXTIME(starttime),"%m-%Y")=date_format(now(),"%m-%Y") group by uid order by total desc LIMIT 10');
        $a = 0;
        $ret = array();
    ...
    public function popularityTotal()
    {
        $rqRank_all = D('Liverecord')->query('SELECT uid,sum(entercount) as total FROM `ss_liverecord` group by uid order by total desc LIMIT 10');
        $a = 0;
        $ret = array();
    ...
    public function wealthDay()
    {
        $richRank_day = D('Coindetail')->query('SELECT uid,sum(coin) as total FROM `ss_coindetail` where type="expend" and date_format(FROM_UNIXTIME(addtime),"%m-%d-%Y")=date_format(now(),"%m-%d-%Y") group by uid order by total desc LIMIT 10');
        $a = 0;
        $ret = array();
        foreach ($richRank_day as $k => $vo) {
            ...
            $ret[$a]['richlevel'] = $richlevel;
            $a++;
        }
        echo json_encode($ret);
    }
    public function wealthWeek()
    {
        $richRank_week = D('Coindetail')->query('SELECT uid,sum(coin) as total FROM `ss_coindetail` where type="expend" and date_format(FROM_UNIXTIME(addtime),"%Y")=date_format(now(),"%Y") and date_format(FROM_UNIXTIME(addtime),"%u")=date_format(now(),"%u") group by uid order by total desc LIMIT 10');
        $a = 0;
        $ret = array();
    ...
    public function wealthMonth()
    {
        $richRank_month = D('Coindetail')->query('SELECT uid,sum(coin) as total FROM `ss_coindetail` where type="expend" and date_format(FROM_UNIXTIME(addtime),"%m-%Y")=date_format(now(),"%m-%Y") group by uid order by total desc LIMIT 10');
        $a = 0;
        $ret = array();
    ...
    public function wealthTotal()
    {
        $richRank_total = D('Coindetail')->query('SELECT uid,sum(coin) as total FROM `ss_coindetail` where type="expend" group by uid order by total desc LIMIT 10');
        $a = 0;
        $ret = array();
        foreach ($richRank_total as $k => $vo) {
            $userinfo = D("Member")->find($vo['uid']);
            //$ret[$a]['userinfo'] = array('nickname'=>$userinfo['nickname'],'curroomnum'=>$userinfo['curroomnum'],'ucuid'=>$userinfo['ucuid']);
            $ret[$a]['userinfo'] = array('nickname'=>$userinfo['nickname'],'curroomnum'=>$userinfo['curroomnum'],'id'=>$userinfo['id']);
            // $emceelevel = getEmceelevel($userinfo['earnbean']);
    ...
    public function giftThisWeek()
    {
        $gifts_thisweek = D('coindetail')->query('SELECT touid,giftid,sum(giftcount) as total FROM `ss_coindetail`,`ss_gift` where `ss_coindetail`.giftid=`ss_gift`.id and date_format(FROM_UNIXTIME(`ss_coindetail`.addtime),"%Y")=date_format(now(),"%Y") and date_format(FROM_UNIXTIME(`ss_coindetail`.addtime),"%u")=date_format(now(),"%u") group by giftid order by total desc limit 30');
        $a = 0;
        $ret = array();
        foreach ($gifts_thisweek as $k => $vo) {
            ...
            $ret[$a]['giftinfo'] = array('name'=>$giftinfo['giftname'],'path'=>$giftinfo['gifticon_25']);
            $ret[$a]['giftcount'] = $vo['total']; 
            $a++;
        }
        echo json_encode($ret);
    }
    public function giftLastWeek()
    {
        $gifts_lastweek = D('coindetail')->query('SELECT touid,giftid,sum(giftcount) as total FROM `ss_coindetail`,`ss_gift` where `ss_coindetail`.giftid=`ss_gift`.id and date_format(FROM_UNIXTIME(`ss_coindetail`.addtime),"%Y")=date_format(now(),"%Y") and date_format(FROM_UNIXTIME(`ss_coindetail`.addtime),"%u")=date_format(now(),"%u")-1 group by giftid order by total desc limit 30');
        $a = 0;
        $ret = array();
        foreach ($gifts_lastweek as $k => $vo) {
            $userinfo = D("Member")->find($vo['touid']);
            //$ret[$a]['userinfo'] = array('nickname'=>$userinfo['nickname'],'curroomnum'=>$userinfo['curroomnum'],'ucuid'=>$userinfo['ucuid']);
            $ret[$a]['userinfo'] = array('nickname'=>$userinfo['nickname'],'curroomnum'=>$userinfo['curroomnum'],'id'=>$userinfo['id']);
            $emceelevel = getEmceelevel($userinfo['earnbean']);
