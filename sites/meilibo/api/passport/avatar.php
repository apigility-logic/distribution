...
if($uid==0 || $uid == -1 || isset($_GET['random'])){
	if (isset($_GET['size']) && $uid == -1) {
		header('Location: ../images/default_touxiang_'.$_GET['size'].'.jpg');
		exit;
	}
...
if(file_exists(dirname(__FILE__).'/'.$avatar)) {
	...
	$avatar_url = 'images/default_touxiang_'.$size.'.jpg';
}
/*
if(empty($random)) {
