...
* 获取上传文件路径
*/
function getUploadPath($uid)
{
    return "/style/avatar/".substr(md5($uid),0,3).'/';
}
/**
 * 组装头像.
 *
 ...
 * @return string
 */
function getAvatar($avatartime, $uid, $size = 'middle')
{
    if(!$avatartime){
        $avatar = '/style/images/default.gif';
    } else if ($size == "yuan") {
        $avatar = '/style/avatar/'.substr(md5($uid),0, 3).'/'.$uid.'.jpg?t=' . $avatartime;
    } else {
        $avatar = '/style/avatar/'.substr(md5($uid),0, 3).'/'.$uid.'_'.$size.'.jpg?t=' . $avatartime;
    }
